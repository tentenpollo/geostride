...
def test_loop_generator_small_graph():
    ...
    G.add_node(2, y=0.01, x=0.0)
    G.add_node(3, y=0.01, x=0.01)
    G.add_node(4, y=0.0, x=0.01)
    
    # Add bidirectional edges with lengths
    edges = [(1, 2), (2, 3), (3, 4), (4, 1), (2, 1), (3, 2), (4, 3), (1, 4)]
    for u, v in edges:
        G.add_edge(u, v, length=1000, highway='residential')
        
    cost_engine = CostFunctionEngine({})
    vibe_engine = VibeEngine({})
    routing = RoutingService(G, cost_engine, vibe_engine)
    
    generator = LoopGenerator(G, routing, cost_engine, vibe_engine)
    
    # 40 mins = 3000m target
    result = generator.generate_loop(1, 40, {"walkability": 1.0})
